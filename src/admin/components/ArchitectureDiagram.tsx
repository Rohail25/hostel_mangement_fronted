...
interface ArchitectureDiagramProps {
  ...
  onDeleteSeat?: (bedId: number) => void;
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
          ...
          <p className="text-2xl font-bold text-blue-900 mt-1">{data.totalRooms}</p>
        </div>
        <div className="bg-green-50 p-4 rounded-xl border border-green-100">
          <p className="text-sm text-green-600 font-medium">Available Seats</p>
          <p className="text-2xl font-bold text-green-900 mt-1">{data.availableSeats}</p>
        </div>
        <div className="bg-red-50 p-4 rounded-xl border border-red-100">
          <p className="text-sm text-red-600 font-medium">Occupied Seats</p>
          <p className="text-2xl font-bold text-red-900 mt-1">{data.occupiedSeats}</p>
        </div>
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
          <p className="text-sm text-slate-600 font-medium">Total Seats</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{data.totalSeats}</p>
        </div>
      </div>
        ...
        {data.floors.map((floor) => (
          ...
          >
            <div className="mb-4 pb-4 border-b border-slate-200">
              <h3 className="text-xl font-bold text-slate-900">Architecture</h3>
              <p className="text-sm text-slate-600 mt-1">{floor.rooms.length} rooms</p>
            </div>

            <div className="mb-3">
              <p className="text-sm font-semibold text-slate-700">Block {floor.floorNumber}</p>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {floor.rooms.map((room) => {
                const occupiedCount = room.seats.filter((seat) => seat.isOccupied).length;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-lg font-bold text-slate-900">{room.roomNumber}</p>
                    </div>
                        ...
                        >
                          <div className="relative group">
                            <motion.div
                              ...
                              title={seat.isOccupied ? `Occupied by ${seat.tenantName || 'Unknown'}` : 'Available'}
                            >
                              {seat.seatNumber}
                            </motion.div>

                            {onDeleteSeat && !seat.isOccupied && seat.bedId && (
                              <button
                                type="button"
                                onClick={() => onDeleteSeat(seat.bedId as number)}
                                className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-white border border-red-200 text-red-600 shadow-sm opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"
                    ...
                    <div className="mt-2 text-center">
                      <p className="text-xs text-slate-500">
                        {occupiedCount}/{room.totalSeats} occupied
                      </p>
                    </div>
                  </motion.div>
                );
